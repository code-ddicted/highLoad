...
const runLoadTest = async () => {
  ...
  const requests = Array.from({ length: 5000 }, () => sendRequest());
  ...
  const otherErrors = results.filter(result => result.status === 'error' && !result.data.error !== ('Balance cannot go negative'));
  ...
  if (successResponses.length > 0) {
    console.log('Sample success:', successResponses.slice(0, 3).map(msg => msg.data.message));
  }
  if (otherErrors.length > 0) {
    console.log('Sample Other Errors:', otherErrors.slice(0, 3).map(err => Object.keys(err.data)));
  }
};
